...
extern bool zuiun::trigger;
extern bool zuiun::aim;
extern bool zuiun::glow;
extern bool zuiun::radar;
extern bool zuiun::flash;
extern bool zuiun::bhop;
...
public:
	...
	void setupGui()
	{
		gui::createHWindow("master", "masterClass");
		gui::CreateDevice();
		gui::CreateImGui();
	...
	void thread_glow_radar(hacks_state* state, hacks_coords* coords, ConsoleIO* io, Internalisation* i) { // Glow ESP - F8; Radar hack - F9
		while (true) {
			if (state->game) {
				bool f8 = glow; // Enemy glow ESP
				bool f9 = radar; // Radar hack
	...
	void thread_panic_mode() {
		while (true) {
			paint_gui();//I do not wanna creat a new thread or rename this due to lazy.
			panic_mode();
			Sleep(1);
		}
	}
